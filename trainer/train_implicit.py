...
class ImplicitTrainingModule(pl.LightningModule):
    ...
    def __init__(self):
        ...
        x_range = y_range = z_range = np.linspace(-0.50, 0.50, 128).astype(np.float32)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(list(self.model.parameters()), lr=0.001)
        return [optimizer]
    ...
    def visualize_prediction(self):
        ...
        sdf_values = torch.cat(sdf_values, dim=0).reshape((128, 128, 128)).numpy()
        vertices, triangles = mc.marching_cubes(sdf_values, 0)
        vertices = vertices / 128 - 0.5
